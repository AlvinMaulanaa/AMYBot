...
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n'
            + 'FN:デ ニ ス\n'
            + 'ORG:Owner Maulanaa Bot;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6288806262251:+62 888-06262-251\n'
            + 'END:VCARD'
       
const vcard1 = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n'
            + 'FN:Caca Cans\n'
            + 'ORG:Co Owner Maulanaa Bot;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6288220018934:+62 882-20018-934\n'
            + 'END:VCARD'

prefix = "#"
name = "Maulanaa BOT"
rdaftar = "@alvinmaulanay_"
rmenu = "@alvinmaulanay_"
botinfo = "@alvinmaulanay_"
limitt = 999999999
memberLimit = 10
ban = []
premium = ["6288806262251@s.whatsapp.net","6285876210829@s.whatsapp.net"]
...
ᴋᴇᴛɪᴋ ${prefix}daftar ᴜɴᴛᴜᴋ ᴍᴇᴍᴜʟᴀɪ ᴍᴇɴɢɢᴜɴᴀᴋᴀɴ ᴍᴀᴜʟᴀɴᴀᴀ ʙᴏᴛ`
	...
	denz.on('chat-update', async (mek) => {
		try {
			...
			const DnsBot = ["6288806262251@s.whatsapp.net"] // ubah aja gapapa
			...
			const ownerNumber = ["6288806262251@s.whatsapp.net","6288220018934@s.whatasapp.net"] // owner number ubah aja
...
denz.sendMessage(from, hasil, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "ketik #menu kak", 'jpegThumbnail': fs.readFileSync('./sticker/dnsnew.webp')}}}})
		...
		reply(`*_「 ʙᴀᴅᴡᴏʀᴅ ᴅᴇᴛᴇᴄᴛᴇᴅ 」_*\nᴍᴀᴀғ *${pushname2}* ᴀɴᴅᴀ ʙᴇʀʙɪᴄᴀʀᴀ ᴋᴏᴛᴏʀ!, ᴀɴᴅᴀ ꜱᴇɢᴇʀᴀ ᴅɪᴋɪᴄᴋ ᴅᴀʀɪ ɢʀᴜᴘ *${groupMetadata.subject}*`)
			...
			if (!isUser) return reply(mess.only.userB)
				dmenu = `╓───「 *_ᴍᴀᴜʟᴀɴᴀᴀ ʙᴏᴛ_* 」
║wa.me/62888062622513
╙───々
╓───「 *_ʙᴏᴛ ɪɴғᴏ_* 」
║❏ ᴄʀᴇᴀᴛᴏʀ: ғxᴄ7 ғᴀʀʜᴀɴ
║❏ ʀᴇᴄᴏᴅᴇ 𝟷 ʙʏ: ᴅɴꜱ ᴛᴇᴀᴍ
║❏ ʀᴇᴄᴏᴅᴇ 𝟸 ʙʏ: ᴀʟᴠɪɴ ᴍᴀᴜʟᴀɴᴀᴀ
║❏ ᴘʀᴇғɪx: ${prefix}
...
quoted: mek, caption: `Hasil Dari *${daerah}*`
  ...
  break
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 *Maulanaa BOT* 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 *Maulanaa BOT* 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat Maulanaa BOT\`\`\``)
					break
                                case 'bcgc':
					denz.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
...
case 'husbu':
        ...
        if (budy.includes("6288806262251")){  // respon tag ubah aja
        reply(`ᴍᴀᴀғ *${pushname2}*, ᴏᴡɴᴇʀ ᴍᴀᴜʟᴀɴᴀᴀ ʙᴏᴛ ᴛɪᴅᴀᴋ ᴍᴇɴᴇʀɪᴍᴀ ᴛᴀɢ!`)
        ...
        if (budy.includes("Alvin")){  // respon tag ubah aja
		...
		if (budy.includes(`Thanks`)) {
                  reply(`ꜱᴀᴍᴀ-ꜱᴀᴍᴀ ᴋᴀᴋ *${pushname2}*, ᴍᴀᴜʟᴀɴᴀᴀ ʙᴏᴛ ᴍɪꜱꜱ ʏᴏᴜ >-<`)
                  }
                  ...
                  if (budy.includes(`maulanaa`)) {
                  reply(`ʜᴀʟʟᴏ ᴋᴀᴋ *${pushname2}*, ᴍᴀᴜʟᴀɴᴀᴀ ʙᴏᴛ ᴅɪꜱɪɴɪ ᴀᴅᴀ ʏᴀɴɢ ʙɪꜱᴀ ᴅɪʙᴀɴᴛᴜ? ᴋᴇᴛɪᴋ *${prefix}menu* ʏᴀ ᴋᴀᴋ`)
                  }
			      if (budy.includes(`Maulanaa`)) {
                  reply(`ʜᴀʟʟᴏ ᴋᴀᴋ *${pushname2}*, ᴍᴀᴜʟᴀɴᴀᴀ ʙᴏᴛ ᴅɪꜱɪɴɪ ᴀᴅᴀ ʏᴀɴɢ ʙɪꜱᴀ ᴅɪʙᴀɴᴛᴜ? ᴋᴇᴛɪᴋ *${prefix}menu* ʏᴀ ᴋᴀᴋ`)
                  }
